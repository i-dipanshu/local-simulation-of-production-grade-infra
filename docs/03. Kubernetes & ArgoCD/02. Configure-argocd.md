...
**Expose the `example-argo-cd-server` service**
   ```sh
   kubectl expose service example-argo-cd-server --type=NodePort --target-port=3000 --name=argo-cd-ui
   ```
...
4. Copy the generated output, excluding any trailing character ie; (%). This is your ArgoCD password for `admin user`. Use `admin` in the id field.
